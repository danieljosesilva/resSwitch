...
""" This program treats raw data of resistive switching measurements in 
order to obtain several key statistical values. For more information consult 
the readeMe.txt file"""
...
class ResSwitch(Tkinter.Tk):
	...
	def __init__(self):
		...
		labelHeader=Tkinter.Label(self.labelFrameLeft,text="cycle       type        tension        resistance      ")
		labelHeader.pack(side='top')
		self.listSetReset = Tkinter.Listbox(self.labelFrameLeft,width=35,yscrollcommand = self.scrollBar.set)
		...
		self.plotFrame = Tkinter.LabelFrame(self.labelFrameRight, text="Cycles")
		...
		self.canvasOne.show()
		self.canvasOne.get_tk_widget().pack(side=Tkinter.TOP, fill=Tkinter.BOTH, expand=1)
		self.toolbarOne = NavigationToolbar2TkAgg(self.canvasOne, self.plotFrame)
		self.toolbarOne.update()
		self.canvasOne._tkcanvas.pack(side=Tkinter.TOP, fill=Tkinter.BOTH, expand=1)


		#initializes matplotlib bottom plot
		self.analysisFrame = Tkinter.LabelFrame(self.labelFrameRight, text="Results")
		self.analysisFrame.pack(fill="both", expand="yes",side='bottom')
		self.figureTwo = Figure(figsize=(5,1), dpi=60)
		self.plotListTwo=[]
		self.plotListTwo.append(self.figureTwo.add_subplot(111))
		self.canvasTwo = FigureCanvasTkAgg(self.figureTwo, self.analysisFrame)
		self.canvasTwo.show()
		self.canvasTwo.get_tk_widget().pack(side=Tkinter.TOP, fill=Tkinter.BOTH, expand=1)
		self.toolbarTwo = NavigationToolbar2TkAgg(self.canvasTwo, self.analysisFrame)
		self.toolbarTwo.update()
		self.canvasTwo._tkcanvas.pack(side=Tkinter.TOP, fill=Tkinter.BOTH, expand=1)


		#Constructs a popup list for top plots
		self.popupPlots = Tkinter.Menu(self, tearoff=0)
		self.popupPlots.add_command(label="Current vs Time", command=self.chanItime)
		self.popupPlots.add_command(label="Current vs Tension",command=self.chanIV)
		self.popupPlots.add_command(label="Tension vs Time", command=self.chanVtime)
		self.popupPlots.add_command(label="Resistance vs Time", command=self.chanRtime)
		self.popupPlots.add_command(label="Resistance vs Wished tension", command=self.chanR)
		...
		self.treatmentMenu.add_command(label="No Set", command=self.noSetWindow)
		self.treatmentMenu.add_command(label="No Reset", command=self.noResetWindow)
		self.analysisMenu.add_cascade(label="Treatment", menu=self.treatmentMenu)

		self.histogramMenu = Tkinter.Menu(self.menuBar, tearoff=0)
		self.histogramMenu.add_command(label="Tension Histogram", command=self.setTenHist)
		self.histogramMenu.add_command(label="Resistance Histogram", command=self.setResHist)
		self.histogramMenu.add_command(label="Current Histogram", command=self.setCurHist)
		self.analysisMenu.add_cascade(label="Histograms", menu=self.histogramMenu)

		self.cycleMenu = Tkinter.Menu(self.menuBar, tearoff=0)
		self.cycleMenu.add_command(label="Resistance vs Cycles", command=self.resPlot)
		self.cycleMenu.add_command(label="Tension vs Cycles", command=self.tensionPlot)
		self.cycleMenu.add_command(label="Current vs Cycles", command=self.currentPlot)
		self.analysisMenu.add_cascade(label="Cycle plots", menu=self.cycleMenu)

		self.scatterPlotMenu = Tkinter.Menu(self.menuBar, tearoff=0)
		self.scatterPlotMenu.add_command(label="Vset and Vreset vs Roff", command=self.tensionRoffPlot)
		self.scatterPlotMenu.add_command(label="Vset and Vreset vs Ron", command=self.tensionRonPlot)
		self.scatterPlotMenu.add_command(label="Ron and Roff vs ion", command=self.resistanceIonPlot)
		self.scatterPlotMenu.add_command(label="Ron and Roff vs ioff", command=self.resistanceIoffPlot)
		self.scatterPlotMenu.add_command(label="Vset and Vreset vs ion", command=self.tensionIonPlot)
		self.scatterPlotMenu.add_command(label="Vset and Vreset vs ioff", command=self.tensionIoffPlot)
		self.analysisMenu.add_cascade(label="Scatter plots", menu=self.scatterPlotMenu)
	...
	def setTenHist(self):
		...
		self.tensionHistoMin.insert(0,min([x for x in self.data.onTension if x != 'no set']+[x for x in self.data.offTension if x != 'no reset']))
		L2 = Tkinter.Label(f12, text="max")
		L2.pack( side = 'left')
		self.tensionHistoMax = Tkinter.Entry(f12, bd =5)
		self.tensionHistoMax.pack(side = 'right')
		self.tensionHistoMax.insert(0,max([x for x in self.data.onTension if x != 'no set']+[x for x in self.data.offTension if x != 'no reset']))
		L3 = Tkinter.Label(f13, text="steps")
		L3.pack( side = 'left')
		self.tensionHistoSteps = Tkinter.Entry(f13, bd =5)
		self.tensionHistoSteps.pack(side = 'right')
		self.tensionHistoSteps.insert(0,(max([x for x in self.data.onTension if x != 'no set']+[x for x in self.data.offTension if x != 'no reset'])
			-min([x for x in self.data.onTension if x != 'no set']+[x for x in self.data.offTension if x != 'no reset']))/0.1)
	...
	def tenHist(self):
		...
		self.bins,self.binsa,self.binsb,onTens,offTens,self.cuma,self.cumb = self.data.tensionHistogram(float(self.tensionHistoMin.get()),
			float(self.tensionHistoMax.get()),float(self.tensionHistoSteps.get()),0)
		self.figureTwo.delaxes(self.plotListTwo[-1])
		self.plotListTwo.append(self.figureTwo.add_subplot(111))
		self.plotListTwo[-1].hist(onTens,self.bins,color='g',alpha=.5,label='set')
		self.plotListTwo[-1].hist(offTens,self.bins,color='r',alpha=.5,label='reset')
	...
	def setResHist(self):
		...
		self.resistanceHistoMax.insert(0,max([x for x in self.data.onResistance if x != 'no set']+[x for x in self.data.offResistance if x != 'no reset'])*100)
		L3 = Tkinter.Label(f13, text="steps")
		L3.pack( side = 'left')
	...
	def resHist(self):
		...
		bins,onRes,offRes,self.cumax,self.cumay,self.cumbx,self.cumby = self.data.ResCum(float(self.resistanceHistoMin.get()),
			float(self.resistanceHistoMax.get()),float(self.resistanceHistoSteps.get()),0)
		self.figureTwo.delaxes(self.plotListTwo[-1])
		self.plotListTwo.append(self.figureTwo.add_subplot(111))
		self.plotListTwo[-1].hist(onRes,bins,color='g',alpha=.5,label='on')
		self.plotListTwo[-1].hist(offRes,bins,color='r',alpha=.5,label='off')
	...
	def curHist(self):
		...
		bins,onCur,offCur,self.cumcax,self.cumcay,self.cumcbx,self.cumcby = self.data.currentHistogram(float(self.currentHistoMin.get()),
			float(self.currentHistoMax.get()),float(self.currentHistoSteps.get()),0)
		self.figureTwo.delaxes(self.plotListTwo[-1])
		self.plotListTwo.append(self.figureTwo.add_subplot(111))
		self.plotListTwo[-1].hist(onCur,bins,color='g',alpha=.5,label='set')
		self.plotListTwo[-1].hist(offCur,bins,color='r',alpha=.5,label='reset')
	...
	def resPlot(self):
		...
		self.figureTwo.delaxes(self.plotListTwo[-1])
		self.plotListTwo.append(self.figureTwo.add_subplot(111))
		self.plotListTwo[-1].plot(range(len(self.onResPlot)),self.onResPlot,'rs',label='on')
		self.plotListTwo[-1].plot(range(len(self.offResPlot)),self.offResPlot,'b^',label='off')
		self.plotListTwo[-1].legend(loc='upper right')
	...
	def tensionPlot(self):
		...
		self.figureTwo.delaxes(self.plotListTwo[-1])
		self.plotListTwo.append(self.figureTwo.add_subplot(111))
		self.plotListTwo[-1].plot(range(len(self.onTenPlot)),self.onTenPlot,'rs',label='set')
		self.plotListTwo[-1].plot(range(len(self.offTenPlot)),self.offTenPlot,'b^',label='reset')
		self.plotListTwo[-1].legend(loc='upper right')
		self.plotListTwo[-1].set_xlabel('cycle')
	...
	def currentPlot(self):
		...
		self.figureTwo.delaxes(self.plotListTwo[-1])
		self.plotListTwo.append(self.figureTwo.add_subplot(111))
		self.plotListTwo[-1].plot(range(len(self.onCurPlot)),self.onCurPlot,'rs',label='set')
		self.plotListTwo[-1].plot(range(len(self.offCurPlot)),self.offCurPlot,'b^',label='reset')
		self.plotListTwo[-1].legend(loc='upper right')
		self.plotListTwo[-1].set_xlabel('cycle')
		self.plotListTwo[-1].set_ylabel('Tension (V)')
		self.plotListTwo[-1].set_yscale('linear')
	...
	#Scatter plot set and reset tensions vs off resistance on the bottom plot
	def tensionRoffPlot(self):
		...
		self.plotListTwo[-1].plot(self.Roffp,self.Vresetp,'b^',label='reset')
		self.plotListTwo[-1].legend(loc='upper right')
	...
	#Scatter plot set and reset tensions vs on resistance on the bottom plot
	...
	#Scatter plot on and off resistance vs set current on the bottom plot
	...
	#Scatter plot on and off resistance vs reset current on the bottom plot
	...
	#Scatter plot set and reset tension vs set current on the bottom plot
	...
	#Scatter plot set and reset tension vs reset current on the bottom plot
	...
	#opens a window to change the values for the resistance and the tension
	...
	def note(self):
		...
		L1.pack( side='top')
		self.commentText = Tkinter.Text(self.frameNote1, height=10, bd =2, yscrollcommand=self.scrollBarNote.set)
	...
	#updates the listSetReset and the self.data accoring to the openWindow function
	def inclu(self):

		if int(self.selectedItem[0])%2 == 0:
			if self.entryR.get() != 'no set' and self.entryV.get() != 'no set' and self.entryI.get() != 'no set':
				self.data.onTension[int(self.selectedItem[0])/2] = float(self.entryR.get())
				self.data.onResistance[int(self.selectedItem[0])/2] = float(self.entryV.get())
				self.data.onCurrent[int(self.selectedItem[0])/2] = float(self.entryI.get())
				self.colorItem[int(self.selectedItem[0])][1] = 0
			else:
				self.data.onTension[int(self.selectedItem[0])/2] = 'no set'
				self.data.onResistance[int(self.selectedItem[0])/2] = 'no set'
			...
			word3 = 'set'
			word4 = '          '
			if type(self.data.onTension[int(self.selectedItem[0])/2]) == types.UnicodeType:
				word5 = 'no set'
				word6 = '          '
				word7 = 'no set'
			else:
				word5 = '%05.2f' % self.data.onTension[int(self.selectedItem[0])/2]
				word6 = '          '
				word7 = str(self.data.onResistance[int(self.selectedItem[0])/2])		
			...
			if self.entryR.get() != 'no reset' and self.entryV.get() != 'no reset' and self.entryI.get() != 'no reset':
				self.data.offTension[int(self.selectedItem[0])/2] = float(self.entryR.get())
				self.data.offResistance[int(self.selectedItem[0])/2] = float(self.entryV.get())
				self.data.offCurrent[int(self.selectedItem[0])/2] = float(self.entryI.get())
				self.colorItem[int(self.selectedItem[0])][1] = 0
			else:
				self.data.offTension[int(self.selectedItem[0])/2] = 'no reset'
				self.data.offResistance[int(self.selectedItem[0])/2] = 'no reset'	
				self.data.offCurrent[int(self.selectedItem[0])/2] = 'no reset'
				self.colorItem[int(self.selectedItem[0])][1] = 1					
			word1 = ' %02d' % float(int(self.selectedItem[0])/2)
			word2 = '          '
			word3 = 'reset'
			word4 = '       '
			if type(self.data.offTension[int(self.selectedItem[0])/2]) == types.UnicodeType:
				word5 = 'no reset'
				word6 = '       '
				word7 = 'no reset'
			else:
				word5 = '%05.2f' % self.data.offTension[int(self.selectedItem[0])/2]
				word6 = '          '
				word7 = str(self.data.offResistance[int(self.selectedItem[0])/2])		
			str1 = word1 + word2 + word3 + word4 + word5 + word6 + word7
			self.listSetReset.delete(int(self.selectedItem[0]))
			self.listSetReset.insert(int(self.selectedItem[0]),str1)
	...
	#updates the listSetReset and the self.data accoring to the noSetWindow function
	def noSetInclu(self):

		if int(self.finalNoSet.get()) < len(self.data.onTension) and int(self.finalNoSet.get()) >= 0 and int(self.initialNoSet.get()) < len(self.data.onTension) and int(self.initialNoSet.get()) >= 0:
			for i in range(int(self.finalNoSet.get())-int(self.initialNoSet.get())+1):
				self.data.onTension[i+int(self.initialNoSet.get())] = 'no set'
				self.data.onResistance[i+int(self.initialNoSet.get())] = 'no set'
				self.data.onCurrent[i+int(self.initialNoSet.get())] = 'no set'	
				...
				word7 = 'no set'
				str1 = word1 + word2 + word3 + word4 + word5 + word6 + word7
				self.listSetReset.delete(int(self.initialNoSet.get())*2+i*2)
				self.listSetReset.insert(int(self.initialNoSet.get())*2+i*2,str1)
				self.listSetReset.itemconfig(int(self.initialNoSet.get())*2+i*2, bg='red', fg='white')
	...
	#updates the listSetReset and the self.data accoring to the noResetWindow function
	def noResetInclu(self):

		if int(self.finalNoReset.get()) < len(self.data.onTension) and int(self.finalNoReset.get()) >= 0 and int(self.initialNoReset.get()) < len(self.data.onTension) and int(self.initialNoReset.get()) >= 0:
			for i in range(int(self.finalNoReset.get())-int(self.initialNoReset.get())+1):
				self.data.offTension[i+int(self.initialNoReset.get())] = 'no reset'
				self.data.offResistance[i+int(self.initialNoReset.get())] = 'no reset'
				self.data.offCurrent[i+int(self.initialNoReset.get())] = 'no reset'	
				...
				word7 = 'no reset'
				str1 = word1 + word2 + word3 + word4 + word5 + word6 + word7
				self.listSetReset.delete(int(self.initialNoReset.get())*2+i*2+1)
				self.listSetReset.insert(int(self.initialNoReset.get())*2+i*2+1,str1)
				self.listSetReset.itemconfig(int(self.initialNoReset.get())*2+i*2+1, bg='red', fg='white')
