...
class setReset:
    ...
    def tensionRoffPlot(self):
        ...
        Roff = []
        for i in range(len(self.onTension)):
            if self.onTension[i] == 'no set' or self.offTension[i] == 'no reset':
                Vset.append(None)
                Vreset.append(None)
                Roff.append(None)
            else:
    ...
    def ResCum(self,minimum,maximum,width,save):
        ...
        onRes=[x for x in self.onResistance if x != "no set"]
        offRes=[x for x in self.offResistance if x != "no reset"]
        bins = 10 ** numpy.linspace(numpy.log10(minimum), numpy.log10(maximum), width)
    ...
    def results(self):
        ...
        avOffTension = numpy.mean([x for x in self.offTension if x != u'no reset'])
        avOnResistance = numpy.mean([x for x in self.onResistance if x != u'no set'])
        avOffResistance = numpy.mean([x for x in self.offResistance if x != u'no reset'])
        ...
        ioff = []
        for i in range(len(self.onTension)):
            if self.onTension[i] != u'no set' and self.offTension[i] != u'no reset':
                Vset.append(self.onTension[i])
                Vreset.append(self.offTension[i])
                Ron.append(self.onResistance[i])
                Roff.append(self.offResistance[i])
        ...
        result += "\n\n\nCorrelation coefficients\n\nCorrelation coefficient Vset/Ron   \t %f" %corrVsetRon
